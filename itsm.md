...
CREATE (slm:Process:Design {name: "Service Level Management"})
...
CREATE (chm:Process:Transition {name: "Change Managment"})
...
CREATE (rdm:Process:Transition {name: "Release & Deployment Management"})
CREATE (pm:Process:Operations {name: "Problem Management"})
...
CREATE (fm:Process:Strategy {name: "Financial Management"})
CREATE (em:Process:Operations {name: "Event Management"})
CREATE (dm:Process:Strategy {name: "Demand Management"})
CREATE (brm:Process:Strategy {name: "Business Relationship Management"})
...
MATCH (p:Process), (s:Source) WHERE p.name = "Problem Management" AND s.name = "Known Error Database" CREATE (p)-[:FILLS]->(s) RETURN p.name, s.name
